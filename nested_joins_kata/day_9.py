...
def _nested_join(primary, secondary, compose):
    return {compose(key, val, _find_value_in_set_by_key(key, secondary)) for (key, val) in primary}


def _find_value_in_set_by_key(key, secondary):
    return next(_extract_value_from_pair(key, secondary), None)


def _extract_value_from_pair(key, secondary):
...
class NestedJoinsTest(unittest.TestCase):
    ...
    def testResultSetHasNonesOnRight_whenJoinLeftNonemptyAndEmptySet(self):
        self.assertEqual(
    ...
    def testResultSetHasNonesOnLeft_whenJoinEmptyAndNonemptySets(self):
        self.assertEqual(
    ...
    def testResultSetHasValues_forCorrespondingKeys_whenJoinRightTwoNonemptyKeys(self):
        self.assertEqual(
